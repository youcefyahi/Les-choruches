        ...
        <div class="bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-hidden">
            ...
            <div class="border-b border-gray-200">
                <nav class="flex space-x-8 px-6">
                    <button v-for="saison in saisons" :key="saison.key" @click="activeSaison = saison.key"
                        class="py-4 px-2 border-b-2 font-medium text-sm transition-colors" :class="activeSaison === saison.key
            ...
            <div class="overflow-y-auto" style="max-height: calc(90vh - 200px);">
                ...
                <div v-else class="p-6">
                    <div v-if="filteredActions.length === 0" class="text-center py-8">
                        ...
                        <button @click="generateStandardActions"
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div v-for="action in filteredActions" :key="action.id"
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center space-x-2">
                                    <input type="checkbox" :checked="action.status === 'terminee'"
                            ...
                            <div class="mt-3 pt-3 border-t space-y-2">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center space-x-1">
                                        <button v-if="action.status !== 'en_cours'"
                                            @click="setActionStatus(action, 'en_cours')"
                                            class="text-blue-600 hover:text-blue-800 text-xs">
                                            Démarrer
                                        </button>
                                        <button v-if="action.status !== 'terminee'"
                                ...
                                <!-- Bouton Guide photos -->
                                <button @click="openGuidePhotos(action)"
                                    class="w-full flex items-center justify-center space-x-1 py-2 px-3 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-xs transition-colors">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z">
                                        </path>
                                    </svg>
                                    <span>Guide photos</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
// Variables pour le guide photos
...
// Méthodes du guide photos
function openGuidePhotos(action) {
